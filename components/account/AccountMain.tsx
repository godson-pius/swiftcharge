...
const AccountMain = () => {
    ...
    useEffect(() => {
        if (typeof window !== "undefined") {
            ...
            setBlurState(!!(Boolean || JSON.parse(`${window.localStorage.getItem("blurstate")}`) === "true"),
            );
        }
    }, [router]);
                ...
                <div className="w-full flex gap-2">
                    ...
                    >
                        ...
                        <div className="flex gap-3 flex-wrap items-center">
                            ...
                            {user?.refs.map((ref) => (
                                <div key={ref.id} className="person flex flex-col items-center flex-grow">
                                    <h2
                                        className={`text-3xl font-semibold cursor-pointer flex items-center ${blurState ? "blur" : null}`}
                                    >
                                        <IoPeopleCircleOutline size={40}/>
                                    </h2>
                                    <p className="ml-1 text-gray-200 text-xs">{ref.username}</p>
                                </div>
                            ))}
